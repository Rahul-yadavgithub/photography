...
const BookingsDashboard = () => {
  ...
  return (
    <div>
      <div className="mb-8">
        <h1 className="text-3xl font-bold tracking-tight text-zinc-900">Bookings & Inquiries</h1>
        <p className="text-sm text-zinc-500 font-medium mt-1">Manage all your customer requests across all service categories.</p>
      </div>
      ...
      <div className="mb-6 flex justify-between items-end">
        <div>
          <h2 className="text-xl font-bold text-zinc-900">Inquiry Categories</h2>
          <p className="text-sm text-zinc-500 mt-1">Categories are automatically generated from incoming requests.</p>
        </div>
      </div>

      {/* Dynamic Category Overview Cards */}
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
        {categories.map((cat, index) => (
          <div 
            key={index}
            onClick={() => navigate(`/bookings/category/${encodeURIComponent(cat.name)}`)}
          ...
          >
            ...
            <div className="relative z-10 mt-6 pt-4 border-t border-zinc-100 flex items-center gap-3">
              {cat.pending > 0 && (
              ...
              {cat.pending === 0 && cat.urgent === 0 && (
                <div className="flex-1 bg-zinc-50 text-zinc-500 text-xs font-bold px-3 py-2 rounded-lg text-center border border-zinc-200">
                  All Caught Up
                </div>
              )}
            </div>
