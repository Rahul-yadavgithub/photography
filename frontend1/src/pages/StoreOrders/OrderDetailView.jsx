  ...
  const handleApprove = async () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        navigate(`/store-orders/category/${encodeURIComponent(categoryName)}`);
      } else {
  ...
  if (!inquiry) {
    return (
      <div className="flex flex-col items-center justify-center py-32">
        ...
        <button onClick={() => navigate('/store-orders')} className="mt-4 px-6 py-2.5 bg-zinc-900 text-white rounded-xl font-bold hover:bg-zinc-800 transition-colors">
          Return to Orders
        </button>
      </div>
    );
  ...
  return (
    <div className="animate-in slide-in-from-right-4 duration-500 relative">
      ...
      {isApproveModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4 backdrop-blur-sm transition-opacity duration-300">
          <div className="bg-white rounded-3xl w-full max-w-md p-8 shadow-2xl animate-in zoom-in-95 duration-300">
            ...
            <h3 className="text-2xl font-bold text-zinc-900 mb-2">Approve Order?</h3>
            <p className="text-zinc-500 mb-8 leading-relaxed">
              Are you sure you want to approve this order? The customer will be notified, and you can proceed with fulfillment.
            </p>
            <div className="flex justify-end gap-3">
              ...
              <button 
                ...
                disabled={isProcessing}
              >
                {isProcessing ? 'Processing...' : 'Yes, Approve Order'}
              </button>
            </div>
          </div>
        </div>
      ...
      <div className="mb-8 flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <button 
            onClick={() => navigate(`/store-orders/category/${encodeURIComponent(categoryName)}`)}
            className="flex items-center gap-1.5 text-zinc-500 hover:text-zinc-900 font-bold text-sm mb-4 transition-colors"
          >
            <ChevronLeft className="w-4 h-4" /> Back to {categoryName} Orders
          </button>
